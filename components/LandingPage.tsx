...
const DOT_COLORS = [
  "rgba(106, 158, 120, 0.65)",
  "rgba(158, 207, 232, 0.70)",
  "rgba(212, 160, 168, 0.55)",
];
...
interface LandingPageProps {
  onBegin: () => void;
  onCalendarConnected: (accessToken: string) => void;
...
export default function LandingPage({
  ...
  async function handleConnectCalendar() {
    if (session?.accessToken) {
      // Already signed in — just fetch events
      onCalendarConnected(session.accessToken);
    } else {
      // Trigger Google OAuth — page reloads after sign-in
      await signIn("google");
    }
  }
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-8 max-w-2xl mx-auto">
        ...
        <h1
          ...
          style={{ fontSize: "clamp(1.8rem, 7vw, 6rem)", letterSpacing: "0.28em", color: textPrimary }}
        ...
        <AnimatePresence>
          {tod && (
            <motion.p
              ...
              style={{ fontSize: "0.78rem", letterSpacing: "0.18em", color: textSecondary }}
            >
        ...
        <motion.p
          ...
          style={{ fontSize: "clamp(1.1rem, 2.5vw, 1.5rem)", color: textSecondary }}
        >
        ...
        <motion.button
          ...
          whileHover={{ scale: 1.04, boxShadow: "0 10px 36px rgba(74, 124, 90, 0.38)" }}
          whileTap={{ scale: 0.97 }}
          onClick={onBegin}
          className="px-14 py-4 rounded-full font-body text-xs tracking-[0.22em] uppercase overflow-hidden transition-shadow duration-300 mb-4"
          style={{
            color: textPrimary,
        ...
        {/* Calendar connect — subtle, optional */}
        <motion.button
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          ...
          className="flex items-center gap-2 font-body text-xs tracking-[0.14em]"
          style={{
            color: calendarConnected ? "rgba(74, 124, 90, 0.9)" : textSecondary,
            opacity: 0.65,
            background: "none",
            border: "none",
            cursor: "pointer",
